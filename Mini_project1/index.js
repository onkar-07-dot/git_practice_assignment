...
function getascard(id,title,url,thumb){

    return `
    
        <div class='card'>
            <div class="card_img">
                <img src="${url}" alt="${title}'s image"></img>
            </div>
            <div class="class_body">
            <div class="card_item card_id">${id} </div>
                <h3 class="card_item card_title">${title}</h3>
                
                <a href= "${thumb}"class="card_item card_link">read More</a>
            </div>
        </div>
        `
...
main.innerHTML = data.map(item =>
    getascard(item.id, item.title, `https://picsum.photos/600?random=${item.id}`, `https://picsum.photos/150?random=${item.id}`)
).join("");
